...
class purchase_day_book_report(report_sxw.rml_parse):
    ...
    def _purchase_details(self, obj):
        ...
        self.cr.execute("select ai.id from account_invoice ai "\
                    "WHERE (ai.date_invoice >= %s) AND (ai.date_invoice <= %s) AND ai.type='in_invoice' AND ai.state != 'draft' AND ai.branch_id = %s ", (self.start_date, self.end_date,self.branch_id))
    ...
    def _get_grand_total(self, cat_id):
        ...
        return total
report_sxw.report_sxw('report.purchase.day.book', 'purchase.day.book.wizard', 'addons/purchase_day_book_report/report/purchase_daybook_report.mako', parser=purchase_day_book_report, header=False)
