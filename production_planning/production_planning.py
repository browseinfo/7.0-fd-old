...
class production_planning(osv.Model):
    ...
    _columns = {
        ...
        'state': fields.selection([
            ...
            ('cancel', 'Cancel'),
            ('done', 'Done'), ], 'Status'),
        'order_line': fields.one2many('production.planning.line', 'order_id', 'Order Lines', states={'confirm': [('readonly', False)]}),
    ...
    def create(self, cr, uid, vals, context=None):
        vals['plan_ref'] = 'Plan Reference/' + self.pool.get('ir.sequence').get(cr, uid, 'production.planning')
        return super(production_planning, self).create(cr, uid, vals, context=context)
...
class production_planning_line(osv.Model):       
    ...
    _columns = {
        ...
        'order_id': fields.many2one('production.planning', 'Order Reference'),
